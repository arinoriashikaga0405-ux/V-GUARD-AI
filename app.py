...
# 1. KONFIGURASI HALAMAN (STANDARD V-GUARD)
...
if 'db_nasabah' not in st.session_state:
    st.session_state.db_nasabah = [
        ...
        {"ID": 102, "W2aktu": "2026-03-28", "Pelanggan": "Jaya", "Bisnis": "Bengkel Berkah", "Usaha": "Otomotif", "Paket": "BASIC", "Harga": 1500000, "Status": "🔴 Menunggu"}
    ]
...
<style>
    ...
    .audit-box { background-color: #f1f3f4; border-left: 5px solid #1a73e8; padding: 15px; border-radius: 5px; }
</style>
""", unsafe_allow_html=True)

# 3. SIDEBAR (LOCKED STANDARD)
with st.sidebar:
    st.markdown("<h1 style='color: #1E3A8A;'>🛡️ V-GUARD AI</h1>", unsafe_allow_html=True)
    if os.path.exists("erwin.jpg"): 
        st.image("erwin.jpg", use_container_width=True)
        st.caption("<p style='text-align:center;'>Erwin Sinaga | Senior Business Leader</p>", unsafe_allow_html=True)
    st.write("---")
...
# --- MENU 2: VISI, MISI & ROI ---
    ...
    st.caption("🟢 ROI Positif Terjamin")
...
# --- MENU 5: AKSES TERBATAS (DENGAN AUDIT GOOGLE STUDIO) ---
    ...
    if not st.session_state.admin_akses_terbuka:
        ...
        with h1: st.header("⚙️ Panel Operasional V-Guard AI")
        with h2:
            if st.button("🔒 LOGOUT"):
                st.session_state.admin_akses_terbuka = False
                st.rerun()
        ...
        with tab2:
            ...
            st.markdown("""
            <div class="audit-box">
                <b>Ringkasan Audit Bulanan (Sync with Gemini AI):</b><br>
                • Skor Integritas Bisnis: <b>98.5%</b><br>
                • Efisiensi Penyelamatan Modal: <b>Tinggi</b><br>
                • Status Laporan Rugi Laba: <b>Verified by V-Guard</b>
            </div>
            """, unsafe_allow_html=True)
            
            # Simulasi Visualisasi Audit
            audit_data = pd.DataFrame({
                'Minggu': ['W1', 'W2', 'W3', 'W4'],
                'Indikasi Fraud': [2, 1, 0, 0],
                'Penyelamatan (Jt)': [1.5, 2.2, 3.0, 2.8]
            })
            st.line_chart(audit_data.set_index('Minggu'))
            st.button("Tarik Laporan Full dari Google Studio")

        with tab3:
            st.subheader("📹 Client Visual Verification (CVV)")
            st.info("Koneksi CCTV AI Client: Aktif")
            st.write("- Integrasi CVV: **Online**")

        with tab4:
            st.button("Kirim Invoice Masal")
