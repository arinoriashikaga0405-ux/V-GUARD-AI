...
# --- 5. SIDEBAR NAVIGATION (FOTO CEO TAMPIL & PORTAL KLIEN AKTIF) ---
with st.sidebar:
    st.markdown("<br>", unsafe_allow_html=True)
    # PERSONAL BRANDING FOUNDER (FOTO AKTIF KEMBALI)
    ...
    st.divider()
    # Portal Klien dikembalikan ke menu utama
...
# --- 7. HALAMAN: LAYANAN & INVESTASI (PRODUK & ROI AKTIF KEMBALI) ---
    ...
    paket = [
        ...
        ("V-PRO", "3.5M", "• Monitor POS Online<br>• Integrasi VCS<br>• Audit Video Harian"),
    ...
    c_roi1.error(f"**Potensi Kebocoran (Fraud/Waste):** Rp {bocor:,.0f}/bln")
    c_roi2.success(f"**Penyelamatan V-Guard AI:** Rp {selamat:,.0f}/bln")
    st.info(f"**Total Penyelamatan Tahunan:** Rp {selamat*12:,.0f}")
    st.markdown('</div>', unsafe_allow_html=True)

# --- 8. HALAMAN: PORTAL KLIEN (TAMPIL KEMBALI DI MENU UTAMA) ---
    ...
    with tab1:
        with st.form("login_klien"):
            ...
            if st.form_submit_button("MASUK DASHBOARD"):
                if uid in USER_AKTIF and USER_AKTIF[uid] == upw: st.success("AKTIF. Menghubungkan ke CCTV & Laporan...")
                else: st.error("ID/Password tidak valid.")
    with tab2:
        with st.form("daftar_baru"):
            st.write("Silakan isi data untuk aktivasi:")
...
# --- 9. HALAMAN: ADMIN CONTROL (LOCKED, TERMASUK AI ASSISTANT) ---
    ...
    if pin:
        if hashlib.sha256(pin.encode()).hexdigest() == ADMIN_PWD_HASH:
            st.success("Halo Pak Founder Erwin Sinaga.")
            
            # --- FITUR LAYANAN ADMIN ---
            c_a, c_b, c_c = st.columns(3)
            c_a.button("🔔 Alarm Audit")
            c_b.button("📊 Rugi Laba")
            c_c.button("📹 CCTV Global Link")

            # --- AI ASSISTANT (Powered by Gemini) JANGAN DIPINDAH ---
            st.divider()
