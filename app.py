...
# NOMOR ASLI PAK ERWIN SINAGA
WA_NOMOR = "6282122190885" 

# 2. CSS PREMIUM (Navy, Gold, White)
st.markdown("""
    <style>
...
if halaman == "🌐 Beranda":
    ...
    with c2:
        st.markdown("## PROTEKSI ASET BISNIS")
        st.write("V-Guard mendeteksi kebocoran dana secara real-time dan mengirimkan Alarm Merah langsung ke WhatsApp Anda.")
        st.info("🎯 Fokus pada Profit, Biarkan V-Guard Menjaga Aset Anda.")

    st.divider()
    st.markdown("<h3 style='text-align:center'>Pilih Paket Layanan</h3>", unsafe_allow_html=True)
    ...
    with p3:
        st.markdown('<div class="card-service"><h3>🏢 CORPORATE</h3><h2 style="color:#d4af37">25 Jt</h2><p>Unlimited Outlet<br>Full Integration</p></div>', unsafe_allow_html=True)
        st.link_button("HUBUNGI SAYA", wa_link("CORPORATE"))

# ==========================================
# HALAMAN LAIN (Tetap Terpasang)
# ==========================================
elif halaman == "🤖 AI Auditor":
    st.title("🤖 AI Auditor Engine")
    st.info("Fitur upload data untuk deteksi fraud.")

elif halaman == "📝 AI Meeting Summarizer":
    st.title("📝 AI Meeting Summarizer")
    st.info("Ubah transkrip rapat menjadi Action Plan.")

elif halaman == "🔐 Panel WA Admin":
    st.title("🔐 Panel Komunikasi WA")
    # Logika panel WA admin
