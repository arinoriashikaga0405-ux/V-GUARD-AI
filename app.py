...
if 'db' not in st.session_state:
    st.session_state.db = {"klien_demo": {"pkt": "V-GROW", "tag": 5500000, "due": "2026-04-05"}}
...
# 3. SIDEBAR (DIPERSIALKAN AGAR TIDAK ERROR)
with st.sidebar:
    ...
    # Navigasi Teks Pendek
    m_list = ["Beranda", "Login"]
    if st.session_state.role == "admin": m_list = ["Beranda", "Klien", "Scanner"]
...
# 4. HALAMAN BERANDA (PROFIL & PAKET - LOCKED)
if menu == "Beranda":
...
# 6. FITUR LAIN
    ...
    st.markdown('<div class="v-head"><h1>TAGIHAN</h1></div>', unsafe_allow_html=True)
    k = st.session_state.db.get(st.session_state.user_id, {})
    if k: st.markdown(f'<div class="v-card"><h2 style="color:#FFD700;">Paket: {k["pkt"]}</h2><p>Tagihan: Rp {k["tag"]:,}</p><p>Jatuh Tempo: {k["due"]}</p></div>', unsafe_allow_html=True)
elif menu == "Klien": st.title("Management Klien")
