...
# 2. DESIGN CSS (SIDEBAR HITAM & KOTAK PROPORSIONAL)
st.markdown("""
<style>
    ...
    .stLinkButton button { width: 100%; background-color: #FFD700 !important; color: #000 !important; font-weight: bold; height: 35px; font-size: 12px !important; border-radius: 8px; }
</style>
...
if menu == "🌐 Beranda":
    ...
    with c_roi2:
        ...
        st.info(f"Potensi Kebocoran yang Dapat Dicegah V-GUARD: **Rp {penyelamatan:,.0f} /Bulan**")
    ...
    with p2:
        ...
        st.link_button("PILIH PAKET", WA)
    with p3:
        st.markdown(f'<div class="card-v" style="border: 2px solid #FFD700;"><h4>🚀 V-PRO</h4><div class="price">15 Jt /Bln</div><div class="label">Deskripsi</div><p>Deep AI Fraud Audit.</p><div class="label">Fitur</div><ul><li>Analisis Perilaku</li><li>Max 5 Outlet</li></ul><div class="label">Target</div><p>Franchise</p></div>', unsafe_allow_html=True)
        st.link_button("PILIH PAKET", WA)
    with p4:
...
# 5. HALAMAN MEETING LAB
    ...
    txt_input = st.text_area("Tempel transkrip rapat di sini:", height=250)
    if st.button("🚀 ANALISIS STRATEGIS"):
        if txt_input:
            with st.spinner("Menganalisis..."):
                try:
                    res = model.generate_content(f"Rangkum poin strategis dan action plan: {txt_input}")
                    st.info(res.text)
                except:
                    st.error("Gagal memproses AI.")
        else:
