...
# --- SIDEBAR & PROFIL ---
with st.sidebar:
    # PERBAIKAN NAMA FILE FOTO: Disesuaikan menjadi erwin.jpg
    foto_path = "erwin.jpg" 
    
    if os.path.exists(foto_path):
        foto_profil = Image.open(foto_path)
        st.image(foto_profil, use_column_width=True)
    else:
        st.warning(f"⚠️ File '{foto_path}' tidak ditemukan.")
        st.markdown("""
        <div style="display: flex; justify-content: center; align-items: center; 
                    background-color: #262730; height: 150px; border-radius: 10px; margin-bottom: 10px;">
            <span style="font-size: 50px; color: #afb1b6;">👤</span>
        </div>
...
# 1. HALAMAN VISI & MISI (TEKS LENGKAP 200+ KATA)
if st.session_state['current_page'] == 'Dashboard Home':
    st.header("Visi & Misi")
    
    st.markdown("""
    <p style="color: #6d7d8e;">
    ...
    # Layout dengan foto Erwin di sisi teks (Sesuai draf yang Anda inginkan)
    ...
    with col_txt:
        ...
        st.markdown("""
        <p style="color: #b0b3b8; line-height: 1.6;">
...
# 2. HALAMAN LAYANAN & INVESTASI (TIDAK ADA PERUBAHAN STRUKTUR)
