...
# --- 1. KONFIGURASI ENGINE AI (SOP: WAJIB ADA) ---
...
# CSS Premium (SOP: Visual Kartu Putih Bersih)
st.markdown("""
    <style>
    .product-card {
...
with st.sidebar:
    ...
    menu = st.radio("STRATEGIC NAVIGATOR", ["Visi & Misi", "Produk & Layanan", "Admin Control Center"])

# --- 4. LOGIKA HALAMAN ---

if menu == "Visi & Misi":
    st.header("Visi & Misi Strategis (200 Kata)")
    st.markdown("""
    <div class="mission-box">
    <b>Visi: Menjadi Jangkar Kepercayaan Digital Global</b><br>
    V-Guard AI Intelligence bervisi menciptakan ekosistem bisnis yang sepenuhnya transparan, aman, dan berintegritas tinggi melalui inovasi teknologi digital. Kami hadir untuk mendigitalisasi kepercayaan (Digitizing Trust), memastikan setiap pemilik usaha memiliki ketenangan pikiran total melalui validasi kejujuran sistem secara real-time. Kami bercita-cita menjadi standar emas global dalam layanan "Integrity Assurance", di mana teknologi kecerdasan buatan otonom kami mampu menghapuskan segala bentuk keraguan operasional serta manipulasi data dalam dunia bisnis yang bergerak serba cepat.<br><br>
    
    <b>Misi: Eliminasi Kebocoran & Perlindungan Aset Strategis</b><br>
    Misi kami adalah memberikan perlindungan aset yang tak tertembus melalui lima pilar utama. Pertama, membangun infrastruktur integritas digital yang mampu mengubah etika kerja menjadi data terukur secara akurat. Kedua, menerapkan teknologi Edge Filtering untuk deteksi dini anomali finansial tepat di titik kejadian transaksi. Ketiga, memastikan efisiensi biaya infrastruktur server hingga 20% bagi seluruh mitra kami melalui optimasi komputasi lokal yang cerdas. Keempat, memberikan kedaulatan akses penuh bagi pemilik usaha melalui Command Center terenkripsi yang dapat dipantau secara nasional dari mana saja. Terakhir, kami berkomitmen menjaga disiplin pengembangan perangkat lunak yang ketat sesuai standar operasional baku V-Guard, guna menjaga warisan bisnis Anda dari risiko kecurangan sistemik maupun kelalaian manusia selamanya.
    </div>
    """, unsafe_allow_html=True)
    ...
    packages = {
        ...
        "V-SIGHT": {"akt": "7,5 JT", "bln": "3,5 JT", "target": "🎯 Gudang & Toko", "feat": "• CCTV AI Behavior Analysis<br>• Fire Alarm System Integration"},
        "V-ULTRA": {"akt": "15 JT", "bln": "10 JT", "target": "🎯 Korporasi", "feat": "• The Core Brain AI Central<br>• Forensic AI (Data 1 Tahun)"}
    }
    ...
    for i, (name, info) in enumerate(packages.items()):
        with cols[i]:
            ...
            <div class="product-card">
                ...
                <div class="price-box">
                    <div style="font-weight:bold; font-size:14px;">Aktivasi: {info['akt']}</div>
                    <div style="color:#2563eb; font-weight:800; font-size:18px;">Langganan: {info['bln']}</div>
                </div>
            </div>
            """, unsafe_allow_html=True)
            st.link_button(f"Pilih {name}", f"https://wa.me/{wa_number}?text=Halo%20Pak%20Erwin,%20saya%20pesan%20{name}", use_container_width=True)
    ...
    if pwd == "w1nbju8282":
        # RAHASIA PERUSAHAAN (SOP: HANYA DI ADMIN)
        st.success("📉 EFISIENSI SERVER & API: 20% ACTIVE")
        st.info("🚨 V-GUARD FIRE ALARM: STANDBY")
        st.divider()
